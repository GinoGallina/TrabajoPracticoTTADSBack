...
const userController = {
    ...
    updateUserById: async (req, res) => {
        try {
            ...
            const updatedUser = await User.findOneAndUpdate({ userId: req.params.id }, result.data, { new: true });
            if (!updatedUser) {
                return res.status(404).json({ error: 'User not found' });
            }
    ...
    deleteUserById: async (req, res) => {
        try {
            const updatedUser = await User.findOneAndUpdate({ userId: req.params.id }, { state: 'Disable' }, { new: true });
            if (!updatedUser) {
                return res.status(404).json({ error: 'User not found' });
            }
