...
const reviewSchema = z.object({
  ...
  rate: z.number().min(1, 'Rate must be between 1 and 5').max(5, 'Rate must be between 1 and 5'),
  state: z.enum(['Active', 'Archived']).refine(value => ['Active', 'Archived'].includes(value), { message: 'Review must be a valid type' }).default('Active')
